...
void displayStatus(GtkBuilder *builder,gchar *msg,gboolean withReset) {
  ...
  GtkStatusbar *status=(GtkStatusbar *)gtk_builder_get_object(builder,
                                                              "statusbar1");
  if (status!=NULL) {
    GtkStyleContext *ctx=gtk_widget_get_style_context(GTK_WIDGET(status));
    if (ctx!=NULL) {
        gtk_style_context_remove_class(ctx,joueurs[(joueurId+1)%2]);
...
void on_actionNewGame_activate(GtkAction* action,gpointer data) {
  ...
  if (builder!=NULL) {
    nbCoup=0;
    joueurId=0; // Au tour du joueur 1
    displayJoueur(builder);
    ...
    if (GTK_IS_CONTAINER(grid)) {
      ...
      GList *child;
      for (child=liste;child!=NULL;child=g_list_next(child)) {
        gtk_widget_set_sensitive(child->data,TRUE);
        GtkStyleContext *ctx=gtk_widget_get_style_context(child->data);
        if (ctx!=NULL) {
          gtk_style_context_remove_class(ctx,"joueur1");
          gtk_style_context_remove_class(ctx,"joueur2");
          gtk_style_context_remove_class(ctx,"win");
        }
      }
      if (liste!=NULL) g_list_free(liste);
    }
...
void on_actionAbout_activate(GtkAction* action,gpointer data) {
  GtkBuilder *builder=GTK_BUILDER(data);
  if (builder!=NULL) {
    GtkWidget *about=(GtkWidget *)gtk_builder_get_object(builder,"aboutdialog1");
    if (about!=NULL) {
...
static void startApplication(GtkApplication* app,gpointer data) {
  ...
  if (builder!=NULL) {
    ...
    if (window!=NULL) {
      ...
      on_actionNewGame_activate(NULL,builder);
    }
  }
}
///////////////////////////////////////////////////////////////////////////////
...
int main(int argc,char *argv[]) {
  GtkApplication *app=gtk_application_new("fr.iutbeziers.tictactoe-5",
